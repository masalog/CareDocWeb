...
CloudFront を唯一の公開エンドポイントとし、静的配信（S3）と API（Lambda）を
...
ブラウザ
   │ HTTPS
   ▼
CloudFront（唯一の公開入口・HTTPS終端・CDN・ルーティング）
...
| API Gateway | API のゲートウェイ・使用量制御 | ビジネスロジック |
...
| 段階 | 方式 | 備考 |
|------|------|------|
| 理想 | CloudFront〜API Gateway 間でキー付与（ブラウザに露出しない） | CloudFront Functions 等で実現 |
| デモ段階 | フロントの fetch でキーを付与（割り切り） | キーはレート制限・コスト保護が主目的 |

> APIキーは「認証」ではなく「使用量制御・コスト保護」。データ保護は Phase 5 の認証（Supabase Auth + JWT）で実現する。
...
最終的に CloudFront を唯一の公開エンドポイントとして両者を統合する。
...
```
   ...
   └───────────────────┘
       ...
       ▼ OAC                    ▼
┌───────────────────┐   ┌───────────────────┐
│   S3 バケット      │   │   API Gateway      │  ← REST API + 使用量プラン
...
| **API Gateway (REST API)** | API のゲートウェイ | LambdaRestApi（全パスをLambdaにプロキシ） |
| **使用量プラン + APIキー** | 使用量制御・コスト保護 | 10req/秒、日次1000req、APIキー必須 |
...
| 出力名 | 内容 |
|--------|------|
| `DistributionUrl` | CloudFrontのURL（`https://xxxxx.cloudfront.net`） |
| `BucketName` | 作成されたS3バケット名 |
...
| ③ API公開 | API Gateway（API Key + レート制限） | ✅ 完了 |
...
| CloudFront Functions で `x-api-key` 注入 | APIキーをブラウザに露出させず付与（設計書の理想形） |

> 疎通統合を先に通したため、上記2点は動作に影響しない独立した仕上げ作業として後続で実施できる。
...
**構成（最もシンプルな CloudFront経由 × GitHub Actions cron）:**

```
GitHub Actions（5分ごとの cron）
    │ curl
    ▼
...
| 要素 | 内容 |
|------|------|
| `GET /api/health` | `HealthController`。`connection.isValid(2)` でDB接続を軽く確認し `{"status":"ok","db":"up"}` を返す。DBエラーでも200を返しウォーム維持を優先（重いクエリは投げない） |
| `.github/workflows/warmup.yml` | `schedule: "*/5 * * * *"`（5分ごと）で CloudFront URL の `/api/health` を `curl` で叩く。`workflow_dispatch` で手動実行も可 |
| キャッシュ | `/api/health` は `/api/*` ビヘイビア（CACHING_DISABLED）に含まれるため毎回 Lambda に到達（`X-Cache: Miss from cloudfront` で確認済み） |

**間隔の判断：5分ごと（GitHub Actions cron の最短）**
...
- コスト：月約8,640回。Lambda無料枠（月100万リクエスト）の約0.86%で無料枠内。
  EventBridge不要でAWS構成変更もなし（`/api/*` に含まれるため）。

> ⚠️ 注意：GitHub Actions の cron は混雑時に数分遅延することがある（正確な5分刻みではない）。
> ウォームアップ用途なら許容範囲。public repo なら Actions は完全無料、private でも無料枠内。
...
- 認証：Supabase Auth（JWT検証）でデータ保護
- CI/CD：GitHub Actions で `cdk deploy` を自動化

---

## 補足：将来の拡張

- **独自ドメイン + ACM証明書**: Route 53 + ACM でカスタムドメイン（例: `caredoc.example.com`）に対応可能
- **API統合**: CloudFront のビヘイビアで `/api/*` を API Gateway に振り分け、フロントとAPIを同一ドメインで配信（CORS不要化）
- **CI/CD**: GitHub Actions で `cdk deploy` を自動化
